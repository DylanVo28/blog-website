...
export const paymentConfig = registerAs('payment', () => ({
  ...
  ocbQr: {
    bankCode: process.env.OCB_QR_BANK_CODE ?? '970448',
    bankName: process.env.OCB_QR_BANK_NAME ?? 'OCB',
    accountNumber: process.env.OCB_QR_ACCOUNT_NUMBER ?? '0123456789',
    accountName: process.env.OCB_QR_ACCOUNT_NAME ?? 'NGUYEN VAN A',
    template: process.env.OCB_QR_TEMPLATE ?? 'compact2',
    minAmount: Number(process.env.OCB_QR_MIN_AMOUNT ?? 10000),
    maxAmount: Number(process.env.OCB_QR_MAX_AMOUNT ?? 5000000),
    expireMinutes: Number(process.env.OCB_QR_EXPIRE_MINUTES ?? 15),
    allowedAmounts: parseAllowedAmounts(process.env.OCB_QR_ALLOWED_AMOUNTS, [
      10000,
      20000,
      50000,
      100000,
